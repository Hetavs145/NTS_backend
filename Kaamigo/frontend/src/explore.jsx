...
import { Outlet, NavLink, Link, useSearchParams, useNavigate } from "react-router-dom";
					...
					<aside className="space-y-6">
						<div className="bg-white p-6 rounded-xl shadow-md">
							...
							<div className="mt-3 grid grid-cols-2 gap-2">
								...
								<button onClick={() => { setQuery(""); setCategory(""); setStatus(""); setLocation(""); setRating(1); setPrice(500); navigate('/explore'); }} className="w-full border border-purple-600 text-purple-700 py-2 rounded hover:bg-purple-50 transition">
									Reset
								</button>
							</div>
						</div>
							...
							{[...Array(5)].map((_, i) => (
								<div key={i} className="flex items-center mb-4">
									...
									<div>
										...
										<Link to="/explore/profile" className="text-sm text-purple-600 hover:underline">
											View Profile
										</Link>
									</div>
								</div>
					...
					<section className="lg:col-span-2 space-y-6">
						<div className="bg-white p-6 rounded-xl shadow-md">
							...
							<div className="text-center mt-4">
								<button className="px-4 py-2 bg-purple-600 text-white rounded hover:bg-purple-700" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
					...
					<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
						{filtered.map((f) => (
							<div key={f.id} className="bg-white p-4 rounded-xl shadow-md">
								...
								<Link to="/explore/profile" className="text-xs text-purple-600 hover:underline">
									View Profile
								</Link>
							</div>
						))}
